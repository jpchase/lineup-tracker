...
export function configureAppStore(storeInstance: RootStore, hydrate: boolean = true): RootStore {
  ...
  if (!initialized || !storeInstance.hasReducer(APP_SLICE_NAME)) {
    debugStore(`configureAppStore: ${initialized ? `state missing for [${APP_SLICE_NAME}], add reducer` : 'first call, do initialization'}`);
    let reducer;
    if (hydrate) {
