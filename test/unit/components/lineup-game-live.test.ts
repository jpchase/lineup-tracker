...
import { cancelSub, cancelSwap, confirmSub, confirmSwap, gameCompleted, selectLiveGameById, selectPlayer, toggleClock } from '@app/slices/live/live-slice.js';
...
describe('lineup-game-live tests', () => {
  ...
  it('shows no game placeholder when no current game', async () => {
    ...
    expect(selectLiveGameById(store.getState()), 'LiveState should have game unset').to.not.be.ok;
